...
- AWS CLIを直接実行してはいけない
- `plugins/aws-cdk-workflow/skills/aws-mcp-server` skill経由でAWS操作を行う
- 破壊的操作はスクリプト内でブロック済み
...
5. 取得した認証情報を環境変数として設定し、MCPスクリプトを実行
   - `cd plugins/aws-cdk-workflow/skills/aws-mcp-server && env AWS_ACCESS_KEY_ID=xxx AWS_SECRET_ACCESS_KEY=yyy AWS_SESSION_TOKEN=zzz pnpm exec tsx index.ts <command> <args>`
...
cd plugins/aws-cdk-workflow/skills/aws-mcp-server && \
