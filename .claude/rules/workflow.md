...
ユーザーから指摘を受けた場合（「これやめて」「なぜ指示に従わなかった？」「常にこうして」等）、同じ問題を繰り返さないようルールファイルを更新する

更新対象

- CLAUDE.md: プロジェクト全体に関わる重要なルール
- .claude/rules/\*.md: 特定ドメインのルール（CDK、Python、AWS操作等）

更新の判断基準

- 1回の指摘: 必要に応じて更新を検討
- 繰り返しの指摘: 必ずルールに追記
- 「常に」「絶対」「必ず」を含む指摘: 即座にルールに追記

注意事項

- CLAUDE.md、.claude/rules/配下の各ファイルは200行以内を維持
- .claude/rules/配下に新規ファイルを追加しない（既存ファイルに追記）
- 既存ルールと矛盾する場合は、新しい指摘を優先して更新
